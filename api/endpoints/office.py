...
@router.post('/office')
def create(request: OfficeSchema, db: Session = Depends(get_db), current_user_info: Tuple[str, str] = Depends(get_user_disable_current), name_company: str = ""):
    name_user, expiration_time = current_user_info
    # Se valida la expiracion del token
    if expiration_time is None:
        return Response(code="401", message="token-exp", result=[])
    ...
    id_sucursal = get_sucursal_by_id(db_company, request.sucursal_id)
    if (not id_sucursal):
        return Response(code="400", message="id sucursal no valido", result=[])

    _office = create_office(db_company, request, name_user)
    return Response(code = "201", message = f"Oficina piso {_office.floor} creada", result = _office).model_dump()


@router.put('/office/{id}')
def update(request: OfficeEditSchema, id: int, db: Session = Depends(get_db), current_user_info: Tuple[str, str] = Depends(get_user_disable_current), name_company: str = ""):
    name_user, expiration_time = current_user_info
    # Se valida la expiracion del token
    if expiration_time is None:
        return Response(code="401", message="token-exp", result=[])
    ...
    _office = update_office(db_company, id, request, name_user)
...
def delete(id: int, db: Session = Depends(get_db), current_user_info: Tuple[str, str] = Depends(get_user_disable_current), name_company: str = ""):
    ...
    _office = delete_office(db_company, id, name_user)
